...
export function SiteShell({
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="ambient-orb ambient-orb--gold absolute left-[-6rem] top-24 h-56 w-56" />
      <div className="ambient-orb ambient-orb--forest absolute right-[-5rem] top-40 h-64 w-64 [animation-delay:1.2s]" />
      <div className="ambient-orb ambient-orb--mist absolute left-[42%] top-[30rem] h-40 w-40 [animation-delay:2.4s]" />

      <div className="relative mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 pb-10 sm:px-6 lg:px-8">
        <header className="sticky top-0 z-20 pt-4 reveal-up">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-4">
                <Link href="/" className="flex items-center gap-3">
                  <div className="floating-emblem flex h-11 w-11 items-center justify-center rounded-full bg-[linear-gradient(135deg,#17372d,#d39244)] text-sm font-bold tracking-[0.24em] text-white shadow-[0_12px_30px_rgba(211,146,68,0.25)]">
                    GG
                  </div>
                  <div>
              ...
              <nav className="flex flex-wrap items-center gap-2 text-sm font-medium text-slate-700">
                <Link className="nav-pill rounded-full px-4 py-2 hover:bg-slate-950/5" href="/">
                  Home
                </Link>
                <Link className="nav-pill rounded-full px-4 py-2 hover:bg-slate-950/5" href="/charities">
                  Charities
                </Link>
                {user ? (
                  <>
                    <Link className="nav-pill rounded-full px-4 py-2 hover:bg-slate-950/5" href="/dashboard">
                      Dashboard
                    </Link>
                    {user.role === "admin" ? (
                      <Link className="nav-pill rounded-full px-4 py-2 hover:bg-slate-950/5" href="/admin">
                        Admin
                      </Link>
                    ) : null}
                  </>
              ...
              <div className="flex flex-wrap items-center gap-2">
                {user ? (
                  ...
                  <>
                    ...
                    <Link
                      className="btn-shimmer rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800"
                      href="/auth/sign-up"
                    >
                      Subscribe now
                    </Link>
        ...
        <footer className="mt-6 rounded-[28px] border border-white/60 bg-white/70 px-6 py-5 text-sm text-slate-600 shadow-[0_10px_40px_rgba(18,36,30,0.08)] backdrop-blur reveal-up reveal-delay-3">
