...
export default async function HomePage() {
  ...
  const tickerItems = [
    `${analytics.activeSubscribers} active subscribers`,
    `${formatCurrency(analytics.totalProjectedCharity, db.config.currency)} projected charity impact`,
    `${getNextDrawLabel(db.config.monthlyDrawDay)} next live draw`,
    `${formatCurrency(analytics.totalPrizePool, db.config.currency)} funded prize pool`,
    `${db.charities.length} searchable charity profiles`,
  ];
  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <Panel className="surface-grid p-8 sm:p-10 reveal-up">
          <div className="space-y-6">
            <span className="eyebrow reveal-up reveal-delay-1">Emotion-led golf platform</span>
            <div className="max-w-3xl space-y-5">
              <h1 className="font-display text-6xl leading-[0.92] text-slate-950 sm:text-7xl reveal-up reveal-delay-2">
                Score five rounds.
                <br />
                Fund real causes.
                <br />
              ...
              <p className="max-w-2xl text-lg leading-8 text-slate-700 reveal-up reveal-delay-3">
            ...
            <div className="grid gap-4 sm:grid-cols-3 reveal-group">
              <div className="spotlight-card rounded-[24px] border border-slate-950/8 bg-white/75 p-4">
            ...
            <div className="flex flex-wrap gap-3 reveal-up reveal-delay-4">
              <Link
                className="btn-shimmer rounded-full bg-slate-950 px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"
        ...
        <div className="grid gap-6 reveal-group">
            ...
            <div className="mt-6 rounded-[26px] bg-white/8 p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                {(currentDraft?.numbers ?? [7, 14, 22, 31, 39]).map((number) => (
                  <div
                    key={number}
                    className="number-orb flex h-14 w-14 items-center justify-center rounded-full border border-white/14 bg-white/10 text-lg font-semibold"
                  >
                    {number}
                  </div>
                ))}
      ...
      <section className="metric-marquee reveal-up reveal-delay-2 rounded-[30px] border border-white/60 bg-white/70 px-4 py-4 shadow-[0_18px_60px_rgba(18,36,30,0.08)] backdrop-blur">
        <div className="metric-marquee-track gap-4">
          {[...tickerItems, ...tickerItems].map((item, index) => (
            <div
              key={`${item}-${index}`}
              className="rounded-full border border-slate-950/8 bg-white/80 px-5 py-3 text-sm font-semibold uppercase tracking-[0.16em] text-slate-700"
            >
              {item}
            </div>
          ))}
      ...
      <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <Panel className="p-7 reveal-up">
          ...
          <div className="mt-8 grid gap-4 reveal-group">
            {[
              ...
              <div key={step} className="spotlight-card flex gap-4 rounded-[24px] border border-slate-950/8 bg-white/75 p-4">
        ...
        <Panel className="p-7 reveal-up reveal-delay-1">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {Object.entries(PLAN_COPY).map(([plan, copy]) => (
              <div
                key={plan}
                className="spotlight-card rounded-[28px] border border-slate-950/8 bg-white/75 p-5 shadow-[0_16px_40px_rgba(20,38,31,0.06)]"
              >
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {db.charities.map((charity) => (
              <Link
                ...
                className="spotlight-card rounded-[28px] border border-slate-950/8 bg-white/75 p-5 transition hover:-translate-y-1 hover:shadow-[0_18px_50px_rgba(20,38,31,0.10)]"
              >
                <Image
                  alt={charity.name}
            ...
            <div className="rounded-[28px] bg-slate-950 px-5 py-6 text-white">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {(latestDraw?.numbers ?? [5, 9, 12, 24, 33]).map((number) => (
                  <span
                    key={number}
                    className="number-orb inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-white/10 text-sm font-semibold"
                  >
                    {number}
                  </span>
                ))}
